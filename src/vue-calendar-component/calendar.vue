...
<template>
    <div class="calendar" >
        <slot name="header"  :prev="prev" :next="next" :changeYear="changeYear" :month="month" :year="year" :day="day">
            <div class="calendar-tools">
                    <span class="calendar-prev" @click="prev" v-if="showPrev">
                    ...
                    <span class="calendar-next"  @click="next" v-if="showNext">
                    <svg width="20" height="20" viewBox="0 0 16 16" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                    <g class="transform-group">
                        <g transform="scale(0.015625, 0.015625)">
                    ...
                    <div class="calendar-info" @click.stop="changeYear">
                        <div class="month">
                            <div class="month-inner" :style="{'top':-(this.month*20)+'px'}">
                                <span v-for="m in datamonths">{{m}}</span>
                            </div>
                        </div>
        ...
        <thead>
            ...
            <slot name="week-header"  :prev="prev" :next="next" :changeYear="changeYear" :month="month" :year="year" :day="day">
                <!-- <tr>
                    <td colspan="7">
        ...
        <tbody class="calendaytbody">
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}" class="days-tr">
            <!--'selected':child.selected,-->
            <td v-for="(child,k2) in day" :class="{'disabled':child.disabled, 'selected':child.selected, 'isSelectStart': child.isSelectStart, 'isSelectEnd': child.isSelectEnd}" @click="select(k1,k2,$event)">
                <span  class="days">{{child.day}}</span>
                <!--<span :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}" class="days">{{child.day}}</span>-->
                ...
                <slot name="day"  :prev="prev" :next="next" :changeYear="changeYear" :month="month" :year="year" :curday="child.day" :child="child">
...
<script>
import { getCurWeekDays, getCurMonth } from './util.js'
import moment from 'moment'
import calendar from './calendar.js'
let lowerMoths = ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月']
...
export default {
    ...
    watch:{
        ...
        isLowerCase : function(val) {
            if (val) {
                this.datamonths = window.navigator.language.toLowerCase() == "zh-cn"?lowerMoths  : val
            }
            this.datamonths = val;
        },
        months: function (val) {
    ...
    mounted() {
        this.init()
        this.datamonths = window.navigator.language.toLowerCase() == "zh-cn"? (this.isLowerCase? lowerMoths : UpMoths ) : this.months;
        this.myInstall()
    },
    methods: {
        init(){
                ...
                if (this.range) { // 范围
                    ...
                    if (this.rangeBegin.length > 0) {
                        ...
                        if (beginTime === stepTime) {
                            options.isSelectStart = true
                        }
                        if (endTime === stepTime) {
                            options.isSelectEnd = true
                        }
                    }
        ...
        select(k1, k2, e) {
            if (e != undefined) e.stopPropagation()
             ...
             if (this.isSelectWeek){
                    ...
                    this.rangeBegin = weekObj.start.split('-').map(item => item - 0)
                    this.rangeBegin[1] = this.rangeBegin[1] - 1
                    this.rangeBeginTemp = this.rangeBegin
                    this.rangeEnd = weekObj.end.split('-').map(item => item - 0)
                    this.rangeEnd[1] = this.rangeEnd[1] - 1
                    // 小于10左边打补丁
                    var begin = new Array()
                    var end = new Array()
                    if(this.zero){
                ...
                this.render(this.year, this.month)
                    ...
                    this.rangeBegin[1] = this.rangeBegin[1] - 1
                    this.rangeBeginTemp = this.rangeBegin
                    this.rangeEnd = month.endDate.split('-').map(item => item - 0)
                    this.rangeEnd[1] = this.rangeEnd[1] - 1
                    // 小于10左边打补丁
                    var begin=[]
                    var end=[]
        ...
        selectYear(value){
            ...
            this.year=value
            this.render(this.year,this.month)
            this.$emit('selectYear',value, this.month, this.day)
        },
        // 返回今天
        setToday(){
            let now = new Date();
        ...
        myInstall () {
            ...
            if (window.touch) {
                ...
                touch.on('.calendaytbody', 'swiperight', function(ev){
                    self.onMyPrev(ev)
                });
            } else {
